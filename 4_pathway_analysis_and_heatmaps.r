...
#Get csv files 
write.csv(res23_24h_LF_sig, file = "LF_sig_DE_genes_MOK023_24HPI_ordered.csv")
write.csv(res23_48h_LF_sig, file = "LF_sig_DE_genes_MOK023_48HPI_ordered.csv")
write.csv(res23_72h_LF_sig, file = "LF_sig_DE_genes_MOK023_72HPI_ordered.csv")
write.csv(res23_168h_LF_sig, file = "LF_sig_DE_genes_MOK023_168HPI_ordered.csv")
# Pathway analysis on bovine genes #####

##run gprofiler - MOK023 ######
library(gProfileR) 

nrow(res23_24h_LF_sig)
head(res23_24h_LF_sig)

names23_24h <- rownames(res23_24h_LF_sig) ## gets the gene name because that’s all we really need at this stage
names23_48h <- rownames(res23_48h_LF_sig)
names23_72h <- rownames(res23_72h_LF_sig)
names23_168h <- rownames(res23_168h_LF_sig) 
length(names23_24h)
head(names23_24h) 

gprofilerresult23_24h <- gprofiler(names23_24h, organism = "btaurus", ordered_query = T, 
                            ...
                            correction_method = "analytical", hier_filtering = "strong", 
...
head(gprofilerresult23_24h) 
gprofilerresult23_48h <- gprofiler(names23_48h, organism = "btaurus", hier_filtering = "strong")
gprofilerresult23_72h <- gprofiler(names23_72h, organism = "btaurus", hier_filtering = "strong")
gprofilerresult23_168h <- gprofiler(names23_168h, organism = "btaurus", hier_filtering = "strong")

write.csv(gprofilerresult23_24h , file="Gprofiler_MOK023_24HPI.csv")
write.csv(gprofilerresult23_48h , file="Gprofiler_MOK023_48HPI.csv")
write.csv(gprofilerresult23_72h , file="Gprofiler_MOK023_72HPI.csv")
write.csv(gprofilerresult23_168h , file="Gprofiler_MOK023_168HPI.csv")
...
# y23_24 is the same as names23_24h but a table version and was created as shown in example below: 
...
# Potentially need to convert this to a vector? But is shouldn't be an issue
names_23_24h_human <- as.vector(names_23_24h_human) 
head(names_23_24h_human) 
names_23_48h_human <- as.vector(names_23_48h_human) 
names_23_72h_human <- as.vector(names_23_72h_human) 
names_23_168h_human <- as.vector(names_23_168h_human) 
...
# Save files 
write.csv(res124_24h_LF_sig, file = "LF_sig_DE_genes_MOK124_24HPI_ordered.csv")
write.csv(res124_48h_LF_sig, file = "LF_sig_DE_genes_MOK124_48HPI_ordered.csv")
write.csv(res124_72h_LF_sig, file = "LF_sig_DE_genes_MOK124_72HPI_ordered.csv")
write.csv(res124_168h_LF_sig, file = "LF_sig_DE_genes_MOK124_168HPI_ordered.csv")

head(res124_24h_LF_sig) 
names124_24h <- rownames(res124_24h_LF_sig) ## gets the gene name because that’s all we really need at this stage
names124_48h <- rownames(res124_48h_LF_sig)
names124_72h <- rownames(res124_72h_LF_sig)
names124_168h <- rownames(res124_168h_LF_sig) 
length(names124_24h)
head(names124_24h) 

gprofilerresult124_24h <- gprofiler(names124_24h, organism = "btaurus", ordered_query = T, 
                                     significant = T, exclude_iea = F, underrep = F, evcodes = F, 
                                     region_query = F, max_p_value = 1, min_set_size = 0, 
                                     max_set_size = 0, min_isect_size = 0, 
                                     correction_method = "analytical", hier_filtering = "strong", 
                                     domain_size = "annotated", custom_bg = "",numeric_ns = "", 
                                     png_fn = NULL, include_graph = F, src_filter = NULL) 
#all default options are used above 
#gives output for gene ontology, kegg, reactome 
head(gprofilerresult124_24h) 
gprofilerresult124_48h <- gprofiler(names124_48h, organism = "btaurus", hier_filtering = "strong")
gprofilerresult124_72h <- gprofiler(names124_72h, organism = "btaurus", hier_filtering = "strong")
gprofilerresult124_168h <- gprofiler(names124_168h, organism = "btaurus", hier_filtering = "strong")

write.csv(gprofilerresult124_24h , file="Gprofiler_MOK124_24HPI.csv")
write.csv(gprofilerresult124_48h , file="Gprofiler_MOK124_48HPI.csv")
write.csv(gprofilerresult124_72h , file="Gprofiler_MOK124_72HPI.csv")
write.csv(gprofilerresult124_168h , file="Gprofiler_MOK124_168HPI.csv") 

# all MOK124 gene lists to human ##### 

# Creation of y124_24 is shown as an example in the MOK023 section 
...
# Heatmap for CODED top 10 KEGG pathways 
library("pheatmap")
